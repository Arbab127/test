...
async def play(client, m: Message):
    ...
    if replied:
        if replied.audio or replied.voice:
            await m.delete()
            huehue = await replied.reply("**✧ درحال اجرای درخواست**")
...
""",
        ...
        if len(m.command) < 2:
            await m.reply("لطفا بررسی کنید که کلمه را درست تایپ کرده اید ")
        else:
            await m.delete()
            huehue = await m.reply("**✧عبارات جستجوی شما با هیچ ورودی مطابقت نداشت**")
            query = m.text.split(None, 1)[1]
            search = ytsearch(query)
            if search == 0:
                await huehue.edit("`چیزی برای پرس و جو داده شده پیدا نشد`")
            else:
                songname = search[0]
                url = search[1]
...
""",
        ...
        if len(m.command) < 2:
            await m.reply(
                "** لطفا بررسی کنید که کلمه را درست تایپ کرده اید **"
            )
        else:
            await m.delete()
            huehue = await m.reply("** عبارات جستجوی شما با هیچ ورودی مطابقت نداشت **")
            ...
            if search == 0:
                await huehue.edit(
                    "**لطفا نام آهنگ های خود را بررسی کنید**"
                )
            else:
                songname = search[0]
                url = search[1]
